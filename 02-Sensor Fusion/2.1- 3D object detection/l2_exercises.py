...
def compute_precision_recall(det_performance_all, conf_thresh=0.5):
    ...
    print("TP = " + str(true_positives) + ", FP = " +
          str(false_positives) + ", FN = " + str(false_negatives))
    ...
    print("precision = " + str(precision) + ", recall = " +
          str(recall) + ", conf_thres = " + str(conf_thresh) + "\n")
...
def pcl_to_bev(lidar_pcl, configs, vis=True):
    ...
    _, idx_height_unique = np.unique(
        lidar_pcl_hei[:, 0:2], axis=0, return_index=True)
    ...
    idx_intensity = np.lexsort(
        (-lidar_pcl_cpy[:, 3], lidar_pcl_cpy[:, 1], lidar_pcl_cpy[:, 0]))
