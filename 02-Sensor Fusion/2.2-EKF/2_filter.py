...
class Filter:
    ...
    def F(self):
        ...
        return np.matrix([[1, 0, self.dt, 0], [0, 1, 0, self.dt], [0, 0, 1, 0], [0, 0, 0, 1]])
...
def run_filter():
    ...
    for i in range(1, 101):
        ...
        # visualization
        ax.scatter(float(x[0]), float(x[1]), color='green',
                   s=40, marker='x', label='track')
        ax.scatter(float(z[0]), float(z[1]), color='blue',
                   marker='.', label='measurement')
        ax.scatter(float(gt[0]), float(gt[1]), color='gray',
                   s=40, marker='+', label='ground truth')
        ax.set_xlabel('x [m]')
        ax.set_ylabel('y [m]')
        ax.set_xlim(0, 10)
        ax.set_ylim(0, 10)
