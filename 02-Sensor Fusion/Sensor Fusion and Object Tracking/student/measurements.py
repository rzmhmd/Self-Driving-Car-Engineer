...
class Sensor:
    ...
    def __init__(self, name, calib):
        ...
        if name == 'lidar':
            ...
            self.sens_to_veh = np.matrix(calib.extrinsic.transform).reshape(
                4, 4)  # transformation sensor to vehicle coordinates
    ...
    def get_hx(self, x):
        ...
        if self.name == 'lidar':
            ...
            # - return h(x)
            ############
            veh_position = np.ones((4, 1))
            veh_position[0:3] = x[0:3]
            sens_position = self.veh_to_sens*veh_position
            hx = np.zeros((2, 1))
            hx[0, 0] = self.c_i-self.f_i*sens_position[1]/sens_position[0]
            hx[1, 0] = self.c_j-self.f_j*sens_position[2]/sens_position[0]
            return hx
