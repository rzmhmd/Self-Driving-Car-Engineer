...
int main()
{
	...
	Vox_filter.setInputCloud(scans[0]);
	Vox_filter.setLeafSize(0.1, 0.01, 0.01);
	typename pcl::PointCloud<PointT>::Ptr cloudFiltered(new pcl::PointCloud<PointT>);
	...
	while (!viewer->wasStopped())
	{
		...
		{
			if (matching == Icp)
				transform = ICP(mapCloud, cloudFiltered, pose, 5); // TODO: change the number of iterations to positive number
