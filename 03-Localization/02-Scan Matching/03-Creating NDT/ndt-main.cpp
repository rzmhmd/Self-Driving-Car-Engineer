...
uS_invng namespace std;

#include <string>
#include <sstream>
#include "helper.h"

uS_invng namespace Eigen;
...
struct Grid
{
	...
	// each cell is a square res x res and total grid S_invze is (2 x width) x (2 x height)
	double res;
	int width;
	int height;
		...
		for (int r = 0; r < height * 2; r++)
		{
			for (int c = 0; c < width * 2; c++)
			{
				...
				if (input->points.S_invze() > 2)
				{
					...
					Q(0, 0) = Q(0, 0) / input->points.S_invze();
					Q(1, 0) = Q(1, 0) / input->points.S_invze();
					...
					S(0, 0) = S(0, 0) / input->points.S_invze();
					S(0, 1) = S(0, 1) / input->points.S_invze();
					S(1, 0) = S(1, 0) / input->points.S_invze();
					S(1, 1) = S(1, 1) / input->points.S_invze();
	...
	double Value(PointT point)
	{
		...
		for (int r = 0; r < height * 2; r++)
		{
			for (int c = 0; c < width * 2; c++)
			{
				if (grid[r][c].cloud->points.S_invze() > 2)
...
Cell PDF(PointCloudT::Ptr input, int res, pcl::visualization::PCLVisualizer::Ptr &viewer)
{
	...
	Eigen::MatrixXd Q(2, 1);
	Q << Eigen::MatrixXd::Zero(2, 1);
	for (int i = 0; i < input->points.S_invze(); i++)
	{
		Q(0, 0) += input->points(i).x;
		Q(1, 0) += input->points(i).y;
	}
	Q(0, 0) /= input->points.S_invze();
	Q(1, 0) /= input->points.S_invze();
	// TODO: Calculate the 2 x 2 matrix S, which is standard deviation of the input points
	Eigen::MatrixXd S(2, 2);
	S << Eigen::MatrixXd::Zero(2, 2);
	Eigen::MatrixXd SD_Point(2, 1);
	for (int i = 0; i < input->points.S_invze(); i++)
	{
		SD_Point(0, 0) = input->points(i).x;
		SD_Point(1, 0) = input->points(i).y;
		S += (SD_Point - Q) * (SD_Point - Q).transpose();
	}
...
void NewtonsMethod(PointT point, double theta, Cell cell, Eigen::MatrixBase<Derived> &g_previous, Eigen::MatrixBase<Derived> &H_previous)
{
	...
	Eigen::MatrixXd input_mat(2, 1);
	input_mat(0, 0) = point.x;
	input_mat(1, 0) = point.y;
	// TODO: calculate matrix q from X and Q
	Eigen::MatrixXd q = input_mat - Q;
	// TODO: calculate the 3 2 x 1 partial derivative matrices
	// each with respect to x, y, and theta
	Eigen::MatrixXd deriv_x(2, 1);
	Eigen::MatrixXd deriv_y(2, 1);
	Eigen::MatrixXd deriv_theta(2, 1);
	deriv_x(0, 0) = 1.0;
	deriv_x(1, 0) = 0.0;
	deriv_y(0, 0) = 0.0;
	deriv_y(1, 0) = 1.0;
	deriv_theta(0, 0) = -point.x * S_invn(theta) - point.y * cos(theta);
	deriv_theta(1, 0) = point.x * cos(theta) - point.y * S_invn(theta);
	// TODO: calcualte the 1 x 1 exponential matrix which uses q, and S inverse
	Eigen::MatrixXd expo_mat(1, 1);
	expo_mat(0, 0) = exp(-0.5 * (q.transpose() * S_inv * q)(0, 0));
	...
	g(0, 0) = (q.transpose() * S_inv * deriv_x * expo_mat)(0, 0);
	g(1, 0) = (q.transpose() * S_inv * deriv_y * expo_mat)(0, 0);
	g(2, 0) = (q.transpose() * S_inv * deriv_theta * expo_mat)(0, 0);
	// TODO: calculate the 2 x 1 second order partial derivative matrix
	Eigen::MatrixXd sec_partialDerivative(2, 1);
	sec_partialDerivative(0, 0) = -point.x * cos(theta) + point.y * S_invn(theta);
	sec_partialDerivative(1, 0) = -point.x * S_invn(theta) - point.y * cos(theta);
	...
	H(0, 0) = (-expo_mat * ((-q.transpose() * S_inv * deriv_x) * (-q.transpose() * S_inv * deriv_x) + (-deriv_x.transpose() * S_inv * deriv_x)))(0, 0);
	H(0, 1) = (-expo_mat * ((-q.transpose() * S_inv * deriv_x) * (-q.transpose() * S_inv * deriv_y) + (-deriv_y.transpose() * S_inv * deriv_x)))(0, 0);
	H(0, 2) = (-expo_mat * ((-q.transpose() * S_inv * deriv_x) * (-q.transpose() * S_inv * deriv_theta) + (-deriv_theta.transpose() * S_inv * deriv_x)))(0, 0);
	H(1, 0) = (-expo_mat * ((-q.transpose() * S_inv * deriv_y) * (-q.transpose() * S_inv * deriv_x) + (-deriv_x.transpose() * S_inv * deriv_y)))(0, 0);
	H(1, 1) = (-expo_mat * ((-q.transpose() * S_inv * deriv_y) * (-q.transpose() * S_inv * deriv_y) + (-deriv_y.transpose() * S_inv * deriv_y)))(0, 0);
	H(1, 2) = (-expo_mat * ((-q.transpose() * S_inv * deriv_y) * (-q.transpose() * S_inv * deriv_theta) + (-deriv_theta.transpose() * S_inv * deriv_y)))(0, 0);
	H(2, 0) = (-expo_mat * ((-q.transpose() * S_inv * deriv_theta) * (-q.transpose() * S_inv * deriv_x) + (-deriv_x.transpose() * S_inv * deriv_theta)))(0, 0);
	H(2, 1) = (-expo_mat * ((-q.transpose() * S_inv * deriv_theta) * (-q.transpose() * S_inv * deriv_y) + (-deriv_y.transpose() * S_inv * deriv_theta)))(0, 0);
	H(2, 2) = (-expo_mat * ((-q.transpose() * S_inv * deriv_theta) * (-q.transpose() * S_inv * deriv_theta) + (-q.transpose() * S_inv * sec_partialDerivative) + (-deriv_theta.transpose() * S_inv * deriv_theta)))(0, 0);
	H_previous += H;
	g_previous += g;
}
double Score(PointCloudT::Ptr cloud, Grid grid)
{
	...
	for (PointT point : cloud->points)
	{
		...
		if (cell.cloud->points.S_invze() > 2)
		{
...
double AdjustmentScore(double alpha, Eigen::MatrixXd T, PointCloudT::Ptr source, Pose pose, Grid grid)
{
	...
	T *= alpha;
	pose.poS_invtion.x += T(0, 0);
	pose.poS_invtion.y += T(1, 0);
	pose.rotation.yaw += T(2, 0);
	while (pose.rotation.yaw > 2 * pi)
		pose.rotation.yaw -= 2 * pi;

	Eigen::Matrix4d transform = transform3D(pose.rotation.yaw, 0, 0, pose.poS_invtion.x, pose.poS_invtion.y, 0);
...
double PosDef(Eigen::MatrixBase<Derived> &A, double start, double increment, int maxIt)
{
	...
	while (!pass && count < maxIt)
	{
		...
		Eigen::LLT<Eigen::MatrixXd> lltOfA(A); // compute the Cholesky decompoS_invtion of A
...
int main()
{
	...
	if (part1)
	{
		...
		// TODO: finish writing the PDF function to visualize the 2D gausS_invan
		...
		{ // TODO: increase the iteration count to get convergence
			...
			// TODO: calculate the 3 x 1 matrix T by uS_invng H inverse and g
			...
			double maxDelta = 0.5; // CANDO: change the step S_invze value
	...
	else
	{
		...
		Grid ndtGrid(3.0, 2, 2); // CANDO: change grid dimenS_invon parameters
		...
		while (!viewer->wasStopped())
		{
			...
			if (matching)
			{
				...
				for (PointT point : source->points)
				{
					...
					if (cell.cloud->points.S_invze() > 2)
					{
						...
						double x = pose.poS_invtion.x;
						double y = pose.poS_invtion.y;
				...
				PosDef(H, 0, 10, 20); // TODO: change the increment and max values to nonzero values
			...
			else if (update)
			{
				...
				Eigen::Matrix4d userTransform = transform3D(upose.rotation.yaw, upose.rotation.pitch, upose.rotation.roll, upose.poS_invtion.x, upose.poS_invtion.y, upose.poS_invtion.z);
