...
vector<int> NN(PointCloudT::Ptr target, PointCloudT::Ptr source, Eigen::Matrix4d initTransform, double dist)
{
	...
	int source_indx = 0;
	for (int i = 0; i < transformSource->points.size())
	{
		// PointT sourcePoint = transformSource->points[i];
		vector<int> TargetIndx;
		vector<float> TragetRadius;
		if (KDTree.radiusSearch(transformSource->points[i], dist, TargetIndx, TragetRadius) > 0)
		{
			associations.push_back(TargetIndx[0]);
		}
		else
		{
